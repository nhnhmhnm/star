...
기준일: 2026-09-08. 유지보수와 기능 추가를 위한 설계 기준이다. 현재는 기획만 수정하며 클래스·서버·패키지를 실제로 만들지 않는다. 프론트엔드 TypeScript + React + Vite, 백엔드 Python + FastAPI를 유지한다.
...
| 원칙 | 적용 |
|---|---|
| 책임 분리 | 지도 표시, 검색, 밤 판정, 하늘 렌더링, 기상 조회를 각각 분리 |
...
| 단일 상태 소유자 | 관측 위치는 웹 상태가, SDK 리소스는 어댑터가 소유. 동일 상태를 여러 클래스에 중복 저장하지 않음 |

모든 함수에 인터페이스나 클래스를 만들지는 않는다. 외부 시스템·시간·수명주기처럼 교체 또는 테스트 격리가 필요한 경계부터 도입한다. 데이터베이스가 없는 현재 단계에는 Repository·Unit of Work·범용 BaseService 등을 추가하지 않는다.
...
| 밤 정책·좌표 계산 | h <= -18°·경도 정규화·단위 변환 | 순수 함수 |
...
관측 위치는 명시적 최종 지도 선택에서만 바뀐다. 지도 이동·검색 결과·기상 제공자의 격자 좌표가 이를 덮어쓰지 않는다. -18° 기준과 현재 시각 전용 정책도 하나의 규칙으로 유지하며 UI별로 중복 작성하지 않는다.

## 4. Python 백엔드 역할 — 도입 시

```mermaid
flowchart LR
    R[FastAPI 라우터] --> S[WeatherService]
    S --> P[WeatherProvider 계약]
    A[OpenMeteoProvider] -. 구현 .-> P
    F[FakeWeatherProvider] -. 구현 .-> P
    S --> C[주입된 현재 시계]
```

라우터는 요청·응답·HTTP 상태 코드를 처리하고, 서비스는 기상 조회 흐름을 조정하며, 제공자는 외부 API 통신·응답 정규화를 맡는다.

| 구성요소 | 책임 |
|---|---|
| FastAPI Router | 좌표 입력 검증·서비스 호출·오류의 HTTP 변환 |
...
검색 API가 필요해지면 같은 책임 분리 방식을 적용하되 기상과 검색을 하나의 만능 서비스로 합치지 않는다. Pydantic 응답 모델과 내부 값이 같고 요구가 단순하면 중복 모델을 강제하지 않는다. 외부 필드가 서비스 내부로 새어 들어올 때 별도 모델로 분리한다.
...
- 서버의 공유 HTTP 연결은 앱 시작/종료에서 생성·정리한다. 요청마다 불필요하게 연결 풀을 만들지 않는다.
- 공유 서비스에는 ‘현재 사용자의 위치’ 같은 변경 가능한 필드를 저장하지 않는다. 좌표·선택 ID는 호출 인자로 전달해 동시 요청이 섞이지 않게 한다.
...
순수 밤 판정은 경계 입력으로 검증하고, 서비스는 fake 제공자·고정 시계로 성공·지연·오류를 검증한다. 실제 제공자 어댑터는 대표 외부 응답을 대상으로 계약 검증을 수행한다. 화면 E2E는 기존 사용자 흐름을 확인한다.
...
최상위 폴더는 docs·backend·frontend로 구분한다. 객체지향의 책임 분리는 각 앱 내부에서 적용한다. 프론트엔드는 frontend/src/features 아래에 기능별 UI·계약·어댑터를 함께 두고, 백엔드는 backend/app/weather처럼 기능 안에서 라우터·서비스·제공자를 구분한다. 전체 코드에서 사용하는 거대한 classes 폴더나 공통 부모 클래스는 만들지 않는다.
