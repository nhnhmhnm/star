...
프론트엔드는 **TypeScript + React + Vite**, 백엔드는 사용자가 선택한 **Python + FastAPI**로 진행한다. 앞선 TypeScript 백엔드 통일안은 대체한다. 실시간 관측자 상태와 공개 앱 설정이 필요하므로 서버는 앱 기반 단계에서 도입한다.
...
| 백엔드          | Python + FastAPI + Uvicorn                                       | 공개 설정·방문 세션·WebSocket presence, 기상/검색 중계·키·캐시 |
| API 데이터 모델 | Pydantic + OpenAPI                                               | 요청·응답 검증, 언어와 독립적인 API 계약                       |
| 데이터베이스    | 초기 도입 없음                                                   | presence는 메모리 TTL, 계정·영속 저장 요구가 생기면 검토       |
...
| 공개 설정을 입력으로 한 태양 고도 판정 | 밤 기준 환경 변수의 검증·공개 설정 제공 |
| 현재 UTC·Stellarium 렌더링             | 기상/검색 API 중계와 응답 정규화        |
| 별자리·시선·줌·기상 표시               | 외부 서비스 실패·시간 초과 처리         |
| 닉네임 입력·관측자 핀·현재 밤 종료 UX  | 임시 방문 세션·실시간 presence·TTL 만료 |

별 좌표를 서버에서 초당 전송하지 않는다. WebSocket은 닉네임과 근사 관측 위치의 접속 상태만 전달한다. 지도용 브라우저 키는 출처/API 제한을 적용하는 공개 클라이언트 키이고 서버용 비밀 키와 구분한다.
...
│   │   ├── core/              설정·공유 자원 조립
│   │   ├── presence/          방문 세션·WebSocket·TTL 저장소
...
│   │   ├── pages/             세계지도·밤하늘 페이지
│   │   ├── features/          세션·presence·지도·검색·밤 판정·하늘·기상 기능
...
프론트엔드는 기능별로 관련 UI·Hook·계약·어댑터와 단위 테스트를 가까이 둔다. 예를 들어 features/sky-viewer에 SkyEngine 계약과 StellariumAdapter를, features/night-eligibility에 밤 판정 순수 함수를, features/presence에 WebSocket 클라이언트와 핀 화면 모델을 둔다. shared에는 실제로 여러 기능이 공유하는 항목만 둔다. 백엔드는 app/presence 안에서 router.py·service.py·store.py·schemas.py처럼 책임을 나누고, app/weather에서는 provider.py와 providers/open_meteo.py를 추가한다. 작은 기능은 처음부터 여러 빈 파일로 나누지 않는다.
...
2. **백엔드와 공개 설정**: FastAPI 기반, 환경 변수의 밤 기준, 공개 설정 계약
3. **앱 기반과 닉네임**: 방문 세션·공통 좌표·시계·설정 경계·라우트 정의
4. **메인 세계지도**: 지도 탐색·검색·클릭 확정·선택 카드·낮/박명 음영·동일 밤 색
5. **실시간 관측자 핀**: 근사 위치·presence 수명주기·집계 핀·장애 격리
6. **밤하늘 상세 페이지**: 검증한 엔진을 연결하고 시선·줌·별자리·밤 종료·복귀 처리
7. **기상**: 데이터 안내부터, 구름 합성은 검증 후
8. **품질·출시 준비**: 실제 기기·성능·오류·자산·배포 설정
...
환경 설정의 기본값인 태양 중심 기하 고도 -18도 이하를 허용한다.
...
설정된 밤 고도 이하 영역은 같은 색상과 투명도로 표시한다.
...
주석은 코드가 이미 말하는 내용을 반복하지 않고, 역할·이유·제약을 알아야 유지보수할 수 있는 곳에만 작성한다. 함수명과 타입으로 충분히 읽히는 컴포넌트, 단순 스타일, 명확한 값 할당에는 주석을 넣지 않는다.

다음 경우에는 짧은 주석을 남긴다.

- 지도·Stellarium·기상 제공자처럼 외부 SDK나 API의 제약 때문에 코드가 특정 순서를 지켜야 할 때
- 기본 태양 고도 -18도와 환경 설정, 현재 시간 전용, 밤 지역 동일 색상처럼 제품 정책이 계산식에 들어갈 때
- 닉네임·근사 위치 공개, heartbeat·TTL처럼 실시간 상태의 수명주기와 개인정보 범위를 설명해야 할 때
- 리스너 해제, canvas 정리, 요청 취소처럼 수명주기 누락이 버그로 이어질 때
- 테스트용 fake, 고정 시계, 샌드박스 우회처럼 제품 동작과 다른 개발용 장치가 있을 때
- 단위 변환, 좌표 정규화, UTC epoch ms처럼 입력 단위가 헷갈리기 쉬울 때

주석은 한두 문장으로 작성하고, 오래된 주석이 실제 코드보다 더 믿기 쉬운 상태가 되지 않도록 같은 커밋에서 함께 수정한다. 긴 설명이 필요하면 코드 안에 늘어놓기보다 관련 문서로 옮긴다.
