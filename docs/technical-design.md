...
지도 밝기는 가능하면 최대 60초마다 재계산한다. 다만 무료 지도 구현이나 성능 제약이 커지면 세계지도 입장 시각의 UTC 스냅샷으로 밝기장을 고정할 수 있다. 클릭·진입·엔진 준비 완료·탭 복귀의 관측 가능 여부는 항상 현재 시각으로 즉시 재계산한다. 하늘은 매초 판정하며 기준 미달이나 계산 오류 시 하늘을 가리고 중단한다. 로딩 중 기준을 넘은 경우도 첫 프레임 전에 차단한다.
...
현재 UTC와 태양 위치로 각 지리 좌표의 기하 고도를 계산한다. 우선 목표는 세계지도 화면에 머무는 동안 같은 단일 시계를 주기적으로 읽어 밝기장을 다시 계산하는 것이다. 무료 지도 구현이나 성능 제약이 커지면 세계지도 입장 시각의 UTC 스냅샷으로 밝기장을 고정해도 된다. 낮·박명의 연속 밝기 효과는 `h > nightAltitudeThresholdDeg` 범위에 적용하고, 기준 이하이면 **하나의 nightTint와 nightOpacity로 고정**한다. 태양 고도가 더 낮아져도 밤 상태 레이어는 변하지 않는다.

빛 중심은 태양 직하점에 대응하는 지리 위치를 기준으로 하며 현재 태양 위치·계절·투영에 따라 모양이 달라질 수 있다. 밝기 스냅샷 모드에서는 세계지도 입장 시각의 태양 위치를 사용한다. 한국 정오 부근에는 한국을 포함한 낮 영역이 넓게 밝게 보이는 효과를 목표로 한다. 화면 좌표에 고정한 원형 그라데이션을 지도와 무관하게 붙이지 않는다.
...
지도 음영과 카드 계산에는 같은 공개 설정·태양 모델을 사용한다. 지도 음영은 주기 갱신 또는 입장 시각 스냅샷을 선택할 수 있지만, 클릭 판정과 밤하늘 진입은 항상 현재 UTC로 원본 좌표에서 직접 계산한다. 지도 밝기 갱신은 매 프레임으로 고집하지 않고 C13에서 무료 구현·성능·UX를 함께 보고 정한다. 해상도·LOD·날짜변경선 처리와 화면 갱신 비용은 향후 검증한다.
