...
현재 UTC와 태양 위치로 각 지리 좌표의 기하 고도를 계산한다. 세계지도 화면에 머무는 동안에도 같은 단일 시계를 주기적으로 읽어 밝기장을 다시 계산하므로 낮·박명·밤 경계는 실제 시간에 따라 천천히 이동한다. 낮·박명의 연속 밝기 효과는 `h > nightAltitudeThresholdDeg` 범위에 적용하고, 기준 이하이면 **하나의 nightTint와 nightOpacity로 고정**한다. 태양 고도가 더 낮아져도 밤 상태 레이어는 변하지 않는다.
...
지도 음영, 설정된 경계, 카드 계산에는 같은 공개 설정·태양 모델·UTC 스냅샷을 사용한다. 지도 렌더링은 보간해도 클릭 판정은 원본 좌표에서 직접 계산한다. 지도 밝기 갱신은 매 프레임으로 고집하지 않고 사용자에게 실시간으로 느껴지는 주기를 C13에서 측정해 정한다. 해상도·LOD·날짜변경선 처리와 화면 갱신 비용은 향후 검증한다.
