...
따라서 C10 구현은 `MapProvider`와 `PlaceSearchProvider`를 분리하고, 먼저 돈이 나가지 않는 지도 화면을 만든다. 기본 경로는 MapLibre GL JS이며, 토큰 없는 정적 세계지도 fallback도 둔다. 외부 지도 공급자 토큰이 없더라도 닉네임 → 세계지도 → 지도 조작 → 클릭 좌표 확정 흐름을 개발·검증할 수 있어야 한다. 지도 밝기장의 실시간 갱신이 무료 구현이나 성능에 부담이 되면 세계지도 입장 시각의 스냅샷으로 고정한다.
...
- 지도 밝기는 주기 갱신을 우선 검토하되, 무료 지도 구현이나 성능 제약이 크면 입장 시각 스냅샷으로 고정한다. 이 경우에도 클릭 판정과 밤하늘 페이지는 현재 UTC를 사용한다.
